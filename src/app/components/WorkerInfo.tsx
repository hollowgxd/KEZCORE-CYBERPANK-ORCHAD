...
const WorkerInfo = () => {
  ...
  return (
    <div className="flex flex-col items-center gap-10 px-4 lg:px-0 max-w-screen-2xl mx-auto">

      {/* ВЕРХНИЙ БЛОК */}

        {/* КАРТИНКА СЛЕВА */}
        <div className="flex flex-col lg:flex-row justify-center items-start gap-6 lg:gap-12 px-2 lg:px-0 w-screen-2xl mx-auto">
        ...
        <div className="flex flex-col gap-6 max-w-5xl bg-black border-2 border-green-600 text-green-400 rounded-lg p-6 animate-pulse-glow-delayed">
          <div>
            <h2 className="text-3xl font-bold mb-4">Сведения о Работниках</h2>
            <p className="text-sm text-gray-300">
              Здесь отображаются все текущие работники, их зарплаты, <br></br>обслуживаемые клетки и общая яйценоскость.
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
            {workers.map((worker) => (
              <div
                key={worker.id}
                className="relative bg-zinc-900 border-2 border-green-600 text-green-300 rounded-2xl p-6 shadow-xl"
              >
                {/* УДАЛИТЬ */}
                <button
                  onClick={() => deleteWorker(worker.id)}
                  className="absolute top-2 right-2 bg-green-700 text-white px-2 py-1 rounded hover:bg-green-800 text-sm transition-all duration-200 transform font-semibold"
                >
                  ✖
                </button>
                ...
                <div className="w-full flex justify-center mb-4">
                  <img
                    src="/dobriy/worker3.webp"
                    alt="Аватар работника"
                    className="w-full h-full object-cover border-2 border-green-600 rounded"
                  />
                </div>
